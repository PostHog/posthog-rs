...
AI events ride their own lane to `POST /i/v1/ai/events` (`Endpoint::CaptureAi`): a separate background worker, queue, and retry state, started on the first `capture_ai*` call, so multi-MB AI events never delay analytics events. The lane batches by size (about 5 MiB per request) and drops an event locally — logging only its name and byte size — when its serialized properties exceed the backend's 8 MiB ceiling. The immediate variants send oversize events and return the backend's `ai_event_too_big` verdict instead.
...
Two new `ClientOptionsBuilder` options scope the lane: `capture_ai_compression` (unset sends AI bodies uncompressed; `Zstd` is recommended, and `capture_compression` does not apply to this lane) and `capture_ai_max_queue_size` (default 1000, lower than `max_queue_size` because AI events can be multi-MB).
