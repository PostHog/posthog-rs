...
impl Client {
    ...
    pub fn flush(&self) {
        // Request every lane's flush first, then wait on each, so the lanes
        // drain concurrently rather than back to back.
    ...
    /// Capture a single AI event and block until the request completes.
    ///
    ...
    /// one-event batch; see it for full semantics.
    #[must_use = "the delivery outcome should be inspected"]
    pub fn capture_ai_immediate(&self, event: Event) -> Result<CaptureSummary, Error> {
        self.capture_ai_batch_immediate(vec![event], false)
    }

    /// Capture a batch of AI events and block until every request completes,
    ...
    pub fn capture_ai_batch_immediate(
        &self,
        events: Vec<Event>,
    ...
    /// each prepared request loop send/classify, sleeping on the calling thread
    /// between retries. The setup and classification are shared with the async
    /// client; only this loop differs. The analytics lane yields one request;
    /// the AI lane yields one per byte-sized chunk, sent in order, and the first
    /// terminal failure ends the call.
    fn send_immediate(
        &self,
        lane: ImmediateLane,
        events: Vec<Event>,
...
mod teardown_tests {
    ...
    #[test]
    fn ai_lane_is_not_spawned_until_capture_ai_is_called() {
    ...
    #[test]
    fn capture_ai_after_shutdown_does_not_spawn_a_lane() {
        let client = client(lane_test_client_options("http://localhost:0".to_string()));
        client.shutdown();
        client.capture_ai(Event::new("$ai_generation", "user-1"));
    ...
    #[test]
    fn disabled_client_ignores_capture_ai() {
