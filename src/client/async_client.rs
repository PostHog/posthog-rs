...
impl Client {
    ...
    pub async fn get_feature_flags<S: Into<String>>(
        ...
        let decide_endpoint = self.options.api_endpoint.replace("/i/v0/e/", "/decide/?v=3");
    ...
    pub async fn get_feature_flag<K: Into<String>, D: Into<String>>(
        ...
        let response = self.get_feature_flags(distinct_id.into(), groups, person_properties, group_properties).await?;
        Ok(response.feature_flags.get(&key_str).cloned())
    }
