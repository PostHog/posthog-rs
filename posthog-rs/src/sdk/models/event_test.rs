...
fn test_custom_event_builder() {
    ...
    assert!(event_obj["timestamp"]
        .as_str()
        .unwrap()
        .starts_with(&Utc::now().date_naive().to_string()));
    assert_eq!(event_obj["properties"]["custom_prop"], "value");
}

#[test]
fn test_anonymous_event() {
    let event = EventBuilder::new("anon_event").anonymous(true).build();
...
fn test_identify_event() {
    ...
    let event = EventBuilder::identify_event("user123".to_string(), values);
...
fn test_pageview_event() {
    ...
    let event = EventBuilder::pageview_event(
        "user123".to_string(),
    ...
    assert_eq!(event_obj["distinct_id"], "user123");
    assert_eq!(
        event_obj["properties"]["$current_url"],
...
fn test_screen_view_event() {
    ...
    let event =
        EventBuilder::screen_view_event("user123".to_string(), "Dashboard".to_string(), Some(values));
...
fn test_survey_event() {
    ...
    let event = EventBuilder::survey_event(
        "user123".to_string(),
    ...
    assert_eq!(
        event_obj["properties"]["$survey_response"],
        "Great service!"
    );
...
fn test_event_builder_failure_cases() {
    ...
    let event = EventBuilder::new("test").properties(json!(null)).build();
...
fn test_standard_events_failure_cases() {
    ...
    let event = EventBuilder::identify_event("user123".to_string(), json!({}));
    assert!(event["properties"].as_object().unwrap().is_empty());

    // Test pageview with empty URL
    let event = EventBuilder::pageview_event("user123".to_string(), "".to_string(), None);
    assert_eq!(event["properties"]["$current_url"], "");

    // Test screen view with empty name
    let event = EventBuilder::screen_view_event("user123".to_string(), "".to_string(), None);
...
fn test_feature_flag_called_event() {
    let event = EventBuilder::feature_flag_called_event(
        "user123".to_string(),
